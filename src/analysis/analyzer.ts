...
function scoreComplexity(
  ...
  const intentScore = Math.max(...strong.map((i) => INTENT_BASE_COMPLEXITY[i.intent]));
...
export function suggestTier(
  ...
  // Atajo reflex: saludo/agradecimiento dominante y mensaje corto.
  // Nada de LLM para responder "hola".
  if (CONVERSATIONAL.has(primary.intent) && structure.words <= 12 && complexity < 0.15) {
    return 'reflex';
  }
...
export function analyze(input: string): Signals {
  const raw = input ?? '';
  ...
  const top = intents[0]!.score;
  const second = intents[1]?.score ?? 0;
  const margin = top === 0 ? 0 : (top - second) / top;
  const confidence = Math.min(1, Math.max(0.1, 0.35 + margin * 0.5 + Math.min(top, 1.5) * 0.15));
  ...
  return {
    ...
    risk,
    confidence,
    suggestedTier: suggestTier(complexity, intents, structure, risk),
  };
}
